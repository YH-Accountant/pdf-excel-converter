...
export const DOCUMENT_FIELDS: Record<DocumentType, FieldDef[]> = {
  ...
  // 통장 입출금내역: 이체확인증은 개별 입/출금이 아니라 총출금액(totalWithdrawal) 하나만 있으므로
  // 그 값을 표시한다. 개별 입금/출금 열은 이체확인증에서 늘 비어 있어 제외했다.
  bankStatement: [
    { key: 'transactionDate', label: '거래일' },
    { key: 'totalWithdrawal', label: '총출금액' },
  ...
  payroll: [
    ...
    { key: 'totalGrossPay', label: '총지급액합계' },
...
const NUMBER_FIELDS = new Set([
  'supplyValue', 'taxAmount', 'totalAmount', 'deposit', 'withdrawal', 'balance', 'totalWithdrawal',
  'incomeTax', 'localIncomeTax', 'totalPayment', 'totalNetPay', 'totalGrossPay', 'amount',
])

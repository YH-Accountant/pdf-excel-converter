...
7. totalAmount (합계금액)
   - ★ 문서에 인쇄된 합계금액을 그대로 읽을 것
   - ★ 공급가액 + 세액과 맞지 않더라도 인쇄된 값을 우선한다.
     증빙에 적힌 값이 곧 사실이며, 계산이 어긋나는 것 자체가 확인해야 할 사안이다.
     임의로 고쳐서 반환하면 원본의 오류를 발견할 수 없게 된다
   - 합계금액이 인쇄되어 있지 않은 경우에만 공급가액 + 세액으로 계산
   - 숫자만 (콤마 없이)
   - 예: 11000000
...
[유형별 결정적 단서 — 아래 표현이 보이면 해당 유형일 가능성이 매우 높음]
문서에 이런 단서가 있으면 우선 근거로 삼되, 단서가 없다고 해당 유형을 배제하지는 말 것
(OCR이 일부 단어를 놓칠 수 있음). 단서가 없으면 문서 전체 내용으로 종합 판단할 것.
- contract: 조항 번호(제1조, 제2조...), 서명/날인란, 갑·을(또는 위탁자·수탁자) 호칭
- taxInvoice: "전자세금계산서" 명칭, 승인번호, 공급자와 공급받는자 양측의 사업자등록번호, 작성일자
- tradingStatement: 인수자/인계자, 납품, 영수/청구 구분 — 실제 거래가 일어났음을 증빙하는 표현
- bankStatement: 이체일시/거래일시, 출금계좌/입금계좌, 은행명, 이체금액/잔액
- withholdingTax: 귀속연월+지급연월이 함께 표기, 소득구분(근로/사업/기타 등), 징수의무자, 원천징수세액
  ※ 귀속연월 단독으로는 급여대장에도 나오므로, 소득구분·징수의무자 등과 조합해서 판단
- estimate: 유효기간, 견적일, 귀하/귀중 — 거래 성립 전(제안 단계)임을 나타내는 표현
- payroll: 직원별 행 나열, 기본급·수당, 4대보험 공제액, 차인지급액(실지급액)
...
[거래명세서 vs 견적서 구분 기준 — 표 구조가 같으므로 거래의 성립 여부로 판단할 것]
- tradingStatement(거래명세서): 이미 일어난 거래를 증빙 — 인수자/인계자, 납품, 영수/청구 표기
- estimate(견적서): 아직 성립 전의 제안 — 유효기간, 견적일, 귀하/귀중 표기
